...
CREATE POLICY "Insertar perfil propio" ON profiles
...
REVOKE UPDATE (credits, credits_open, credits_indoor, credits_train, credits_pilates) 
  ON TABLE profiles FROM PUBLIC, authenticated, anon;
REVOKE INSERT (credits, credits_open, credits_indoor, credits_train, credits_pilates) 
  ON TABLE profiles FROM PUBLIC, authenticated, anon;
...
CREATE POLICY "Todos pueden leer clases" ON classes
...
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER   -- Excepción estricta controlada
SET search_path = public
AS $$
DECLARE
  v_profile RECORD;
BEGIN
  ...
  RETURN jsonb_build_object(
    'id', v_profile.id,
    'email_fallback', v_profile.email_fallback,
